...
import { DollarSign, Plus, Edit, X, User, TrendingUp, AlertCircle, Clock, ChevronRight, Receipt } from 'lucide-react';

import { recordPayment, getRevenue } from '@/app/actions/revenue';
...
export default function RevenuePage() {
    ...
    const fetchRevenue = async () => {
        setLoading(true);
        try {
            const result = await getRevenue(filter);
            if (result.success) {
    ...
    const handleAddPayment = async () => {
        ...
        try {
            const res = await recordPayment({
                visitId: showPaymentModal.id, // Note: recordPayment expects visitId or paymentId? 
                // Wait, recordPayment in revenue.js creates a NEW payment for a visit.
                // But the UI might be managing an EXISTING payment record.
                // Let's assume recordPayment handles the logic correctly.
        ...
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-gray-400 mt-1">Visit-linked revenue and partial payment tracking</p>
                </div>
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
                    <div className="bg-[#120e1b] border border-white/10 rounded-3xl w-full max-w-lg overflow-hidden shadow-[0_0_50px_rgba(179,97,234,0.2)]">
                        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
                            ...
                            <button
                                onClick={() => { setActiveBill(null); setShowPaymentModal(null); }}
